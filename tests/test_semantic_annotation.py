...
def make_document(
    *,
    ...
    logical_units: tuple[LogicalUnit, ...] | None = None,
    annotations: tuple[SemanticAnnotation, ...] = (),
) -> CanonicalDocument:
    if elements is None:
        elements = (
            element("e1", 0, "Definition: A queue follows FIFO."),
            element("e2", 1, "Ordinary explanation."),
            element("e3", 2, "Ví dụ: enqueue A rồi B."),
        )
    context_nodes = (
        ContextNode(
            id="ctx1",
    ...
    return CanonicalDocument(
        ...
        metadata=DocumentMetadata(title="Structures", language="vi"),
        elements=elements,
...
class SemanticAnnotationTests(unittest.TestCase):
    def test_heuristic_provider_generates_definition_and_orphan_example(self) -> None:
        ...
        self.assertEqual(
            by_target[("e1", SemanticAnnotationType.DEFINITION)].source,
    ...
    def test_topic_group_label_becomes_derived_topic_not_source_heading(self) -> None:
        ...
        units = (
            LogicalUnit(
                id="lu_topic",
                type=LogicalUnitType.TOPIC_GROUP,
                element_ids=("e1",),
                label="Queues",
        ...
        result = SemanticAnnotator(HeuristicSemanticProvider()).annotate(
            make_document(elements=elements, logical_units=units)
        )
    ...
    def test_low_confidence_and_disallowed_types_are_filtered(self) -> None:
        provider = RecordingProvider(
            (
                ...
                SemanticCandidate(
                    target_id="lu1",
                    type=SemanticAnnotationType.CUSTOM,
                    value="custom",
                    confidence=0.99,
                ),
            )
    ...
    def test_per_target_limit_is_policy_not_silent_overflow(self) -> None:
        provider = RecordingProvider(
            tuple(
                SemanticCandidate(
                    target_id="lu1",
                    type=SemanticAnnotationType.KEYWORD,
                    value=f"k{index}",
    ...
    def test_rerun_replaces_same_provider_but_preserves_other_annotations(self) -> None:
        old_same = SemanticAnnotation(
            ...
            source=StructureSource.INFERRED,
            confidence=0.8,
            model_version="recording:0",
            metadata={"semantic_provider": "recording"},
        )
        other = SemanticAnnotation(
            id="other",
            target_id="lu1",
            type=SemanticAnnotationType.NOTE,
            value="manual enrichment",
            source=StructureSource.INFERRED,
            confidence=0.9,
            model_version="other:1",
            metadata={"semantic_provider": "other"},
        )
        provider = RecordingProvider(
            (
                SemanticCandidate(
        ...
        result = SemanticAnnotator(provider).annotate(
            make_document(annotations=(old_same, other))
        )
    ...
    def test_output_is_deterministic_for_same_document_provider_and_policy(self) -> None:
        ...
        self.assertEqual(
            first.document.semantic_annotations[0].id,
            second.document.semantic_annotations[0].id,
        )
        ...
        provider = RecordingProvider(
            (
                SemanticCandidate(
                    ...
                    value="Queue",
                    confidence=0.9,
                    metadata={"semantic_provider": "spoofed"},
                ),
            )
        )
