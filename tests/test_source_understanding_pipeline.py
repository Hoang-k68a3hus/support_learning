...
from source_understanding.schemas.document import ContentRegion, DocumentStructure, ProcessingManifest, SemanticAnnotationType
...
def make_elements():
    return (
        Element(id="e1", type=ElementType.PARAGRAPH, order=0, raw_text="Definition: A queue follows FIFO.", normalized_text="Definition: A queue follows FIFO.", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
        Element(id="e2", type=ElementType.PARAGRAPH, order=1, raw_text="Ordinary explanation.", normalized_text="Ordinary explanation.", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
    )


def make_raw_elements():
    return (
        RawElement(text="Definition: A queue follows FIFO.\r\ncontinued", type_hint="paragraph", order=0, provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test-adapter")),
        RawElement(text="Ordinary explanation.", type_hint="paragraph", order=1, provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test-adapter")),
    )
...
class SourceUnderstandingPipelineTests(unittest.TestCase):
    ...
    def test_processing_timestamp_must_be_timezone_aware(self):
        naive = processing().model_copy(update={"processed_at": datetime(2026, 1, 1)})
        with self.assertRaisesRegex(SourceUnderstandingPipelineError, "timezone-aware"):
            SourceUnderstandingPipeline().understand(document_id="doc1", content_hash=HASH, processing=naive, elements=make_elements())

    def test_source_identity_must_be_canonical(self):
        with self.assertRaisesRegex(SourceUnderstandingPipelineError, "trimmed"):
            SourceUnderstandingPipeline().understand(document_id=" doc1 ", content_hash=HASH, processing=processing(), elements=make_elements())
        with self.assertRaisesRegex(SourceUnderstandingPipelineError, "sha256"):
            SourceUnderstandingPipeline().understand(document_id="doc1", content_hash="bad", processing=processing(), elements=make_elements())
    ...
    def test_semantics_enrich_without_mutating_structural_snapshot(self):
        ...
        self.assertEqual(result.document.semantic_annotations[0].target_id, "e1")
        self.assertEqual(result.document.semantic_annotations[0].type, SemanticAnnotationType.DEFINITION)
    ...
    def test_semantic_stage_cannot_mutate_source_or_structure(self):
        class MutatingSemantic:
            def annotate(self, document):
                return SimpleNamespace(document=document.model_copy(update={"elements": tuple(reversed(document.elements))}))
    ...
    def test_auto_content_regions_are_generated_and_recorded(self):
        elements = (
            ...
            Element(id="e2", type=ElementType.CODE, order=1, raw_text="x = 1", normalized_text="x = 1", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
        )
    ...
    def test_auto_content_regions_can_be_disabled(self):
        ...
        manifest = result.document.processing.configuration["source_understanding_pipeline"]
        self.assertEqual(manifest["content_regions"]["source"], "NONE")

    def test_caller_regions_receive_same_ownership_validation(self):
        region = ContentRegion(id="r1", element_ids=("e1", "e2"), dominant_type="narrative", structure=DocumentStructure(), source=StructureSource.DERIVED, confidence=0.8)
        result = SourceUnderstandingPipeline().understand(document_id="doc1", content_hash=HASH, processing=processing(), elements=make_elements(), regions=(region,))
        self.assertEqual(result.document.regions, (region,))
        self.assertTrue(all(unit.region_id == "r1" for unit in result.document.logical_units))
        manifest = result.document.processing.configuration["source_understanding_pipeline"]
        self.assertEqual(manifest["content_regions"]["source"], "CALLER")

    def test_incomplete_caller_regions_are_rejected_before_assembly(self):
        region = ContentRegion(id="r1", element_ids=("e1",), dominant_type="narrative", structure=DocumentStructure(), source=StructureSource.DERIVED, confidence=0.8)
        with self.assertRaisesRegex(SourceUnderstandingPipelineError, "must cover every canonical element"):
            SourceUnderstandingPipeline().understand(document_id="doc1", content_hash=HASH, processing=processing(), elements=make_elements(), regions=(region,))

    def test_interaction_region_can_promote_document_to_mixed(self):
        elements = (
            Element(id="e1", type=ElementType.PARAGRAPH, order=0, raw_text="Intro", normalized_text="Intro", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
            Element(id="e2", type=ElementType.QUESTION, order=1, raw_text="Why?", normalized_text="Why?", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
            Element(id="e3", type=ElementType.ANSWER, order=2, raw_text="Because.", normalized_text="Because.", provenance=Provenance(source=StructureSource.EXPLICIT, extractor="test")),
        )
        result = SourceUnderstandingPipeline().understand(document_id="doc1", content_hash=HASH, processing=processing(), elements=elements)
    ...
    def test_pipeline_rejects_stage_count_drift_before_assembly(self):
        ...
        with self.assertRaisesRegex(SourceUnderstandingPipelineError, "element_count mismatch"):
            SourceUnderstandingPipeline(profiler=WrongProfiler()).understand(document_id="doc1", content_hash=HASH, processing=processing(), elements=make_elements())
