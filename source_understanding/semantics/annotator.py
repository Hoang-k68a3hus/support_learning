...
class SemanticAnnotationError(ValueError):
    """Semantic provider output cannot be attached safely to CanonicalDocument."""
...
class SemanticAnnotator:
    """Generate optional, provenance-safe semantic annotations after structure parsing."""
    ...
    def __init__(
        ...
        policy: SemanticAnnotationPolicy | None = None,
    ) -> None:
        self._provider = provider
        self._policy = policy if policy is not None else SemanticAnnotationPolicy()
        self._provider_name, self._provider_version = self._validate_provider(provider)
    ...
    def annotate(self, document: CanonicalDocument) -> SemanticAnnotationResult:
        ...
        if not self._policy.enabled:
            return SemanticAnnotationResult(
                ...
                retained_annotation_count=len(document.semantic_annotations),
                target_ids=target_ids,
                coverage=evaluate_semantic_coverage(document, target_ids),
                document=document,
            )
        ...
        try:
            ...
            raise SemanticAnnotationError(
                f"semantic provider {self._provider_name!r} failed: {exc}"
            ) from exc
        ...
        generated = tuple(
            self._to_annotation(document, request_kind, candidate)
            for request_kind, candidate in accepted
        )
        ...
        return SemanticAnnotationResult(
            ...
            accepted_annotation_count=len(generated),
            retained_annotation_count=len(retained),
            replaced_annotation_ids=replaced_ids,
            skipped_low_confidence_count=diagnostics["low_confidence"],
            skipped_disallowed_type_count=diagnostics["disallowed_type"],
            skipped_duplicate_count=diagnostics["duplicate"],
            skipped_target_limit_count=diagnostics["target_limit"],
            target_ids=target_ids,
            coverage=evaluate_semantic_coverage(enriched_document, target_ids),
        ...
        if self._policy.annotate_logical_units:
            for logical_unit in document.logical_units:
                ...
                context_labels = tuple(
                    ...
                    for context_id in logical_unit.context_node_ids
                )
                requests.append(
                    SemanticRequest(
                        target_id=logical_unit.id,
                        target_kind=SemanticTargetKind.LOGICAL_UNIT,
                        text=self._limit_request(text),
                        ...
                        metadata={
                            "structure_source": logical_unit.source.value,
                            "structure_confidence": logical_unit.confidence,
                        },
                    )
                )
            ...
            for element in document.elements:
                ...
                requests.append(
                    SemanticRequest(
                        ...
                        context_labels=tuple(context_labels),
                        metadata={
                            "element_type": element.type.value,
                            "logical_unit_ids": [owner.id for owner in owners],
                        },
                    )
                )
    ...
    def _select_candidates(
    ...
    ) -> tuple[tuple[tuple[SemanticTargetKind, SemanticCandidate], ...], dict[str, int]]:
        ...
        for index, candidate in enumerate(candidates):
            if candidate.target_id not in request_by_target:
                raise SemanticAnnotationError(
                    f"provider {self._provider_name!r} returned annotation for target "
                    f"{candidate.target_id!r} that was not requested"
                )
        ...
        for request in requests:
            ...
            deduplicated: dict[tuple[str, str], tuple[int, SemanticCandidate]] = {}
            for provider_index, candidate in target_candidates:
                key = (candidate.type.value, candidate.value.strip().casefold())
                ...
                if candidate.confidence > previous_candidate.confidence:
                    ...
                    candidate.confidence == previous_candidate.confidence
                    and provider_index < previous_index
                ):
                    deduplicated[key] = (provider_index, candidate)
            ...
            if len(ordered) > self._policy.max_annotations_per_target:
                diagnostics["target_limit"] += (
                    len(ordered) - self._policy.max_annotations_per_target
                )
    ...
    def _retained_annotations(
        self,
        existing: tuple[SemanticAnnotation, ...],
    ) -> tuple[tuple[SemanticAnnotation, ...], tuple[str, ...]]:
        ...
        for annotation in existing:
            same_provider = (
                annotation.metadata.get("semantic_provider") == self._provider_name
            )
    ...
    def _to_annotation(
        ...
        if collision:
            raise SemanticAnnotationError(
                "provider candidate metadata uses reserved semantic keys: "
                f"{sorted(collision)}"
            )
        metadata = {
            **candidate_metadata,
            "semantic_provider": self._provider_name,
            "semantic_provider_version": self._provider_version,
            "semantic_annotator_version": self.version,
            "request_target_kind": target_kind.value,
        }
        value = candidate.value.strip()
        return SemanticAnnotation(
            id=self._annotation_id(document, candidate, metadata),
    ...
    def _rebuild_document(
        ...
        try:
            ...
            raise SemanticAnnotationError(
                f"semantic annotations failed CanonicalDocument validation: {exc}"
            ) from exc
    ...
    @staticmethod
    def _validate_provider(provider: SemanticProvider) -> tuple[str, str]:
    ...
    def _coerce_candidate(candidate: object) -> SemanticCandidate:
        ...
        try:
            ...
            raise SemanticAnnotationError(
                f"semantic provider returned invalid candidate {candidate!r}: {exc}"
            ) from exc
