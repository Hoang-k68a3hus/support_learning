import { Controller, Delete, Get, HttpCode, HttpStatus, Param, Patch, Post, Put, Query, Req, UseGuards, Body } from '@nestjs/common';
...
export class WorkspacesController {
  ...
  @Post()
  create(@CurrentUser() principal: AuthPrincipal, @Body() dto: CreateWorkspaceDto) {
    return this.workspaces.create(principal.userId, dto);
  }

  @Get()
  list(@CurrentUser() principal: AuthPrincipal, @Query() query: ListWorkspacesQueryDto) {
  ...
  @Patch(':id')
  update(@CurrentUser() principal: AuthPrincipal, @Param('id') id: string, @Body() dto: UpdateWorkspaceDto) {
