...
export function validateEnvironment(env: Record<string, unknown>): ValidatedEnvironment {
  ...
  return {
    ...
    OUTBOX_RELAY_INSTANCE_ID: parseControlledName(requireString(env, 'OUTBOX_RELAY_INSTANCE_ID'), 'OUTBOX_RELAY_INSTANCE_ID', 160),
    OUTBOX_RELAY_POLL_INTERVAL_MS: outboxRelayPollIntervalMs,
    OUTBOX_RELAY_BATCH_SIZE: parseBoundedPositiveInteger(requireString(env, 'OUTBOX_RELAY_BATCH_SIZE'), 'OUTBOX_RELAY_BATCH_SIZE', 1, 1000),
    OUTBOX_RELAY_CLAIM_LEASE_MS: outboxRelayClaimLeaseMs,
    OUTBOX_RELAY_MAX_PUBLISH_ATTEMPTS: parseBoundedPositiveInteger(requireString(env, 'OUTBOX_RELAY_MAX_PUBLISH_ATTEMPTS'), 'OUTBOX_RELAY_MAX_PUBLISH_ATTEMPTS', 1, 100),
    ...
    WORKER_PROCESSING_CONCURRENCY: parseBoundedPositiveInteger(requireString(env, 'WORKER_PROCESSING_CONCURRENCY'), 'WORKER_PROCESSING_CONCURRENCY', 1, 128),
    WORKER_LEARNING_CONCURRENCY: parseBoundedPositiveInteger(requireString(env, 'WORKER_LEARNING_CONCURRENCY'), 'WORKER_LEARNING_CONCURRENCY', 1, 128),
    WORKER_MAINTENANCE_CONCURRENCY: parseBoundedPositiveInteger(requireString(env, 'WORKER_MAINTENANCE_CONCURRENCY'), 'WORKER_MAINTENANCE_CONCURRENCY', 1, 128),
    WORKER_SHUTDOWN_GRACE_MS: parseBoundedPositiveInteger(requireString(env, 'WORKER_SHUTDOWN_GRACE_MS'), 'WORKER_SHUTDOWN_GRACE_MS', 1000, 300000),
  };
}
